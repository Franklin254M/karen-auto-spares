const SUPABASE_URL = 'https://pvnyclihnjlplwimnpth.supabase.co';
...
async function loadSharedData() {
  try {
    ...
    const salesResult = await client.from('sales').select('*, products(name, price, cost)').order('created_at', { ascending: false });
    const returnsResult = await client.from('returns').select('*, products(name)').order('created_at', { ascending: false });
...
function renderNavigation() {
  const links = isAdmin() ? [['overview', '⌂ Overview'], ['sales', '↗ Sales & receipts'], ['returns', '↩ Returns & warranty'], ['inventory', '▦ Inventory'], ['purchasing', '＋ Purchasing'], ['customers', '◎ Customers'], ['reports', '▤ Reports'], ['team', '◎ Team access']] : [['overview', '⌂ Overview'], ['sales', '↗ Sales & receipts'], ['returns', '↩ Returns & warranty'], ['inventory', '▦ Available stock']];
...
function overviewPage() {
  const dailySales = todaySales();
  const revenue = dailySales.reduce((sum, sale) => sum + Number(sale.total || 0), 0);
  const profit = dailySales.reduce((sum, sale) => sum + Number(sale.profit || 0), 0);
  const low = products.filter((product) => product.stock <= product.reorder_level);
  const availableStock = products.reduce((sum, product) => sum + Number(product.stock || 0), 0);
  return `<div class="grid metrics"><div class="surface metric"><span>Today's revenue</span><strong>${money(revenue)}</strong><small>${dailySales.length} receipts today, shop-wide</small></div><div class="surface metric"><span>Today's profit</span><strong>${money(profit)}</strong><small>All agents, recorded today</small></div><div class="surface metric"><span>Available stock</span><strong>${availableStock}</strong><small>Units across ${products.length} parts</small></div><div class="surface metric"><span>Returns & warranty</span><strong>${returns.length}</strong><small>Shop-wide cases</small></div></div><div class="surface table-wrap"><div class="panel-head"><div><h3>Today's sales</h3><p class="sub">Sales and profit recorded today, all agents</p></div></div>${profitTable(dailySales)}</div><div class="grid split"><div class="surface"><div class="panel-head"><div><h3>Low-stock purchase list</h3><p class="sub">Shared alerts from inventory</p></div><button class="action" data-online-view="inventory">Open inventory</button></div><div class="list">${low.length ? low.map((product) => `<div class="list-row">${productImage(product)}<div><strong>${escapeHtml(product.name)}</strong><small>${product.stock} left · buy ${product.reorder_level - product.stock} units</small></div><span class="status warn">Buy now</span></div>`).join('') : '<p class="sub">No low-stock parts.</p>'}</div></div><div class="surface"><div class="panel-head"><div><h3>Recent receipts</h3><p class="sub">Shop-wide, most recent first</p></div></div><div class="list">${sales.slice(0, 5).map((sale) => `<div class="list-row"><span class="thumb"></span><div><strong>${escapeHtml(sale.products?.name || 'Part')}</strong><small>${sale.receipt_no} · ${sale.qty} units · ${escapeHtml(sale.agent_name || 'Team member')}</small></div><strong>${money(sale.total)}</strong></div>`).join('')}</div></div></div>`;
}

function expectedProfit() { return sales.reduce((sum, sale) => sum + sale.qty * Number((sale.products?.price || 0) - (sale.products?.cost || 0)), 0); }
function profitTable(rows = sales) { return `<table class="table"><thead><tr><th>Receipt</th><th>Part</th><th>Qty</th><th>Sale total</th><th>Profit made</th><th>Expected profit</th></tr></thead><tbody>${rows.length ? rows.map((sale) => `<tr><td>${escapeHtml(sale.receipt_no)}</td><td>${escapeHtml(sale.products?.name || 'Part')}</td><td>${sale.qty}</td><td>${money(sale.total)}</td><td>${money(sale.profit)}</td><td>${money(sale.qty * Number((sale.products?.price || 0) - (sale.products?.cost || 0)))}</td></tr>`).join('') : '<tr><td colspan="6">No sales recorded today.</td></tr>'}</tbody></table>`; }

function salesPage() { return `<div class="toolbar"><h3>Sales & receipts</h3><button class="primary" id="online-sale">＋ Record sale</button></div><div class="surface table-wrap"><table class="table"><thead><tr><th>Receipt</th><th>Part</th><th>Agent</th><th>Qty</th><th>Total</th><th>Profit made</th><th>Expected profit</th><th>Time</th></tr></thead><tbody>${sales.map((sale) => `<tr><td>${sale.receipt_no}</td><td>${escapeHtml(sale.products?.name || 'Part')}</td><td>${escapeHtml(sale.agent_name || 'Team member')}</td><td>${sale.qty}</td><td>${money(sale.total)}</td><td>${money(sale.profit)}</td><td>${money(sale.qty * Number((sale.products?.price || 0) - (sale.products?.cost || 0)))}</td><td>${new Date(sale.created_at).toLocaleString()}</td></tr>`).join('')}</tbody></table></div>`; }
...
function inventoryPage() { return `<div class="toolbar"><h3>${isAdmin() ? 'Shared inventory' : 'Available stock'}</h3>${isAdmin() ? '<button class="primary" id="online-stock">＋ Receive new stock</button>' : '<span class="sub">Read-only stock view</span>'}</div><div class="surface table-wrap"><table class="table"><thead><tr><th>Part</th><th>Acronym</th><th>Barcode</th><th>Available</th><th>Incoming cost</th><th>Supplier</th><th>Sell price</th><th>Status</th>${isAdmin() ? '<th></th>' : ''}</tr></thead><tbody>${products.map((product) => `<tr><td>${productImage(product)} ${escapeHtml(product.name)}</td><td>${escapeHtml(product.acronym)}</td><td>${escapeHtml(product.barcode)}</td><td>${product.stock}</td><td>${money(product.cost)}</td><td>${escapeHtml(product.supplier || '—')}</td><td>${money(product.price)}</td><td><span class="status ${product.stock <= product.reorder_level ? 'warn' : ''}">${product.stock <= product.reorder_level ? 'Buy now' : 'Healthy'}</span></td>${isAdmin() ? `<td><button class="action" data-edit-product="${product.id}">Edit</button></td>` : ''}</tr>`).join('')}</tbody></table></div>`; }
